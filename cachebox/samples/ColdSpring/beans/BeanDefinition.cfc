...
<cfcomponent name="BeanDefinition" 
	...
	<cffunction name="getBeanInstance" access="public" output="false" returntype="any" 
		...
		<cfset var cachebox = 0 />
		<!--- create this if it doesn't exist --->
		<cftry>
			<cfif isSingleton()>
				<cfif structKeyExists(getBeanFactory(),"getCacheAgent")>
					<cfset cachebox = getBeanFactory().getCacheAgent() />
					<cfset bean = cachebox.fetch(getBeanID()) />
					
					<cfif bean.status>
						<cfset bean = cachebox.store(getBeanID(), createObject("component", getBeanClass()) ) />
					</cfif>
					
					<cfset bean = bean.content />
				<cfelse>
