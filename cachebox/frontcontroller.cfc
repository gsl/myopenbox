...
hint="I am the management application for manually viewing or modifying cache configuration">
	...
	<cffunction name="home" access="public" output="true">
		...
		<cfset var stats = history.getCountDownToTotalDestruction() />
		...
		<cfoutput>
			<div id="home">
				...
				<cfif stats.MinutesRemaining gt 0 and stats.MinutesRemaining lte getConfig().getDestructWarningThreshold()>
					<div class="warning statusmessage">Possible memory failure in #stats.MinutesRemaining# minutes.</div>
				</cfif>
	...
	<cffunction name="agent_list" access="public" output="true">
		...
		<cfargument name="drop" type="string" required="false" default="0" />
		...
		<cfif not qry.recordcount>
			<cfoutput><div class="empty statusmessage">No Agents Registered.</div></cfoutput>
		</cfif>
		
		<cfif len(arguments.agentid)>
			<cfset config = mgr.getAgentConfig(arguments.agentid) />
			<cfset confirm("The agent #config.agent.getAgentName()# is reset.") />
		<cfelseif val(drop)>
			<cfset confirm("Content removed from cache") />
		</cfif>
		
		<h3>&rArr; Agent List</h3>
				...
				<tbody>
					<cfoutput group="appname">
						<cfif qry.context is "application">
							<tr class="appname"><th colspan="6"><a name="#htmleditformat(appname)#">#htmleditformat(appname)#</a></th></tr>
						</cfif>
						<cfoutput group="agentname">
							<cfset agentid = mgr.getAgentID(context,appname,agentname) />
							<cfset config = mgr.getAgentConfig(agentid) />
							<cfset agentid = urlencodedformat(agentid) />
							<tr>
								<td>
									<a href="?event=agent_reset&amp;agentid=#agentid#" class="icon reset" title="Reset"></a>
									<a href="?event=agent_browse&amp;agentid=#agentid#" class="icon detail" title="Browse Content"></a>
									<a href="?event=agent_detail&amp;agentid=#agentid#" title="Agent Details">#htmleditformat(agentname)#</a>
								</td>
								<td>#int(occupancy)#</td>
								<td><span><cfif val(oldest)>#formatAge(oldest)#</cfif></span></td>
								...
								<td>
									...
									#config.evictPolicy# <cfif val(config.evictAfter) gt 0>(#config.evictAfter#)</cfif>
								</td>
							</tr>
						</cfoutput>
					</cfoutput>
	...
	<cffunction name="agent_detail" access="public" output="true">
		...
		<cfoutput>
			...
			<h3>
				&rArr; <a href="?event=agent_list">Agent List</a> 
				&rArr; #formatAgentName(config.agent)# 
			</h3>
	...
	hint="allows the user to view and potentially drop single items from cache">
		...
		<cfoutput>
			<h3>
				&rArr; <a href="?event=agent_list">Agent List</a> 
				&rArr; <a href="?event=agent_detail&amp;agentid=#urlencodedformat(arguments.agentid)#">#formatAgentName(config.agent)#</a>
				&rArr; Content 
			</h3>
			
			<cfif qry.recordcount>
				<cfset pages = getPages(qry.recordcount,arguments.startrow,"?event=agent_browse&amp;agentid=" & urlencodedformat(arguments.agentid)) />
				#pages#
							...
							<tr class="hits_#int(qry.hitCount)#">
								<td>
									#numberformat(currentrow,000)# <cfif qry.hitCount>
										<a href="?event=contentdrop&amp;cachename=#urlencodedformat(qry.cachename)#" class="icon drop" title="Remove From Cache"></a>
									</cfif>
								</td>
								<td>
									<cfif qry.hitCount>
										<a href="?event=contentview&amp;cachename=#urlencodedformat(qry.cachename)#" onclick="return popup(this);" title="View Content" target="_blank">
											#htmleditformat(listlast(qry.cachename,"|"))#
										</a>
									<cfelse>
										#htmleditformat(listlast(qry.cachename,"|"))#
									</cfif>
								</td>
								<td>#formatAge(qry.timeStored)#</td>
								<td>#formatAge(qry.timeHit)#</td>
								<td>#int(qry.hitCount)#</td>
								<td>#int(qry.missCount)#</td>
								<td>#formatFrequency(currentTime-qry.timeStored,qry.hitCount)#</td>
							</tr>
						</cfloop>
					</tbody>
				</table>
	...
	<cffunction name="contentdrop" access="public" output="true" hint="removes a specific item from cache">
		...
		<cflocation url="?event=agent_list&drop=1" addtoken="false" />
	</cffunction>
	...
	<cffunction name="options" access="public" output="true">
		...
		<cfoutput>
			<div>
				...
				<a href="?event=serverreset" class="iconlink reset" style="margin-left:25px;">Reset Server Cache</a>
			</div>
		</cfoutput>
